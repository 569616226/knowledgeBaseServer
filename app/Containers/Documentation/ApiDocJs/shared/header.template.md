...
文档将会介绍怎么简单的使用 RESTful API，包括 用户验证，发送请求，响应，错误信息，调用频率，分页，查询等等。

## **请求头**


有些接口需要用特定的格式发送数据。
通常情况下，所有的接口都希望使用json格式，所以你需要在每一个http请求头部包含 `Accept => application/json` 来确保每次发送的都是一个有效的json格式
...
所有REST API请求都被限制以防止滥用并确保稳定性。
您的应用程序每天可以调用的接口的次数，根据您所做的请求的类型而不同。
